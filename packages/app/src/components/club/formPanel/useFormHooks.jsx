...
export const useFormHooks = ({ toPanel }) => {
  ...
  const isAllFieldsValid = () =>
    // data.serial.trim().length === 4 &&
    // data.number.trim().length === 6 &&
    // data.date_of_issue !== null &&
    // data.division_code.trim().length === 6;
    true;

  const isNothingEmpty = () =>
    // Object.values(data).every((i) =>
    //   typeof i === "string" ? i.trim().length !== 0 : true
    // );
    data.email.trim().length !== 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data: responseData, statusCode } = await api(
        ...
        {
          email: data.email,
          // date_of_issue: Math.floor(data.date_of_issue.getTime() / 1000),
        }
      );
      if (responseData) {
