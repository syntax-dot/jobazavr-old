    ...
    if (body?.code) {
      ...
      if (!findUser) {
        if (!body?.organization_name)
          errorGenerator(Errors.ENTER_ORGANIZATION_NAME);
      ...
      if (body.phone === '79951275110' || body.phone === '79191177388') {
        ...
        const smsReq = await axios.post(
          `http://77.244.223.108:8083/broker-api/send`,
          {
            messages: [
              {
                ...
                sms: {
                  ...
                  ttl: '300',
                  content: {
                    text: `Ваш код подтверждения для входа в личный кабинет: ${random6DigitCode}`,
                  },
                },
              },
            ],
          ...
          {
            headers: {
              Authorization: 'Basic aHR0cF8xMTM3OjNDSlQ2blFS',
            },
          },
        );

        if (smsReq.data !== 'OK') errorGenerator(Errors.BAD_REQUEST);
      }
