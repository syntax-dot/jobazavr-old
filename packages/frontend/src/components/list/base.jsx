...
const WorkList = ({ toPanel }) => {
  ...
  const [, setShowPageHeader] = useRecoilState(getShowPageHeader);
  const [clickedWorkplace, setClickedWorkplace] = useRecoilState(
    getClickedOnWorkPlaceList
  );
  ...
  async function get() {
    ...
    if (initData.onboarding) {
      const { data: marksData } = await api(`jobs/city/${599}?limit=250`);
  ...
  useEffect(() => {
    if (!clickedWorkplace) get();
    if (clickedWorkplace) setClickedWorkplace(false);
  }, []);
  return (
    <>
