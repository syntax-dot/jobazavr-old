...
const MainStack = ({ modal, toBack, routerHooks }) => {
  ...
  useEffect(() => {
    if (modal && modal === "tutorial") {
      localStorage.setItem("first_training", "true");
    }
  }, [modal]);
  ...
  const handleContinueClick = async (skip = false) => {
    if (currentStep === 4 || skip) {
      toBack();
      await localStorage.setItem("first_training", "true");
    } else {
  ...
  return (
    <ModalRoot activeModal={modal} onClose={toBack}>
      <ModalConstructor
        ...
        close={toBack}
      >
        <RegisterModal toModal={routerHooks.toModal} close={toBack} />
      </ModalConstructor>
